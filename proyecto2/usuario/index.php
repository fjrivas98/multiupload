                    ...
                    <tbody>
                        <?php
                            foreach($usuarios as $usuarios) {
                                $nombre = urlencode($usuarios->getNombre());
                                ?>
                                <tr >
                                    <td><input type="checkbox" name="ids[]"  value="<?= $usuarios->getId() ?>" form="fBorrar" /></td>
                                    <td><?php echo $usuarios->getId(); ?></td>
                                    <td><?= $usuarios->getCorreo() ?></td>
                                    <td><?= $usuarios->getAlias() ?></td>
                                    <td><?= $usuarios->getNombre() ?></td>
                                    <td><?php 
                                    if($usuarios->getActivo()===1){
                                        echo 'activo';
                                    }else{
                                        echo 'no activo';
                                    }
                                    ?></td>
                                    <td><?= $usuarios->getFechaAlta() ?></td>
                                    <td><a href="dodelete.php?id=<?= $usuarios->getId() ?>" class = "borrar">Borrar</a></td>
                                    <td><a href="edit.php?id=<?= $usuarios->getId() ?>">Editar</a></td>
                                </tr>
                                <?php
                            }
                        ?>
